...
SAVE_PATH = r"D:\Zainab\FYP\IntelliPlan-AI\images_masks\images_masks\model-epochs"

BATCH_SIZE = 4
EPOCHS = 80
IMAGE_SIZE = 64
LATENT_DIM = 128
MAX_TRAIN_IMAGES = 500   # use first 500 for now
...
class ImageDataset(Dataset):
    def __init__(self, folder, max_images=None, image_size=IMAGE_SIZE):
...
# ========== LOAD DATA ==========
train_set = ImageDataset(TRAIN_PATH, max_images=MAX_TRAIN_IMAGES)
...
def sample_and_save(epoch):
    ...
    with torch.no_grad():
        # 1️⃣ Random samples from latent space
        z = torch.randn(BATCH_SIZE, LATENT_DIM, device=DEVICE)
        gen = decoder(z).cpu()

        # 2️⃣ Real validation samples
        real_imgs = []
        for i, batch in enumerate(val_loader):
            # Ensure img always has 4 dimensions [B, C, H, W]
...
# Force RGB
        ...
        # 4️⃣ Comparison grid (real on top, recon below)
        comparison = torch.cat([real_imgs, recon], dim=0)
        utils.save_image(comparison, os.path.join(SAVE_PATH, f"comparison_epoch{epoch}.png"),
                         nrow=BATCH_SIZE, normalize=True)

        # 5️⃣ Random samples
        grid = utils.make_grid(gen, nrow=4, normalize=True)
        utils.save_image(grid, os.path.join(SAVE_PATH, f"sample_epoch{epoch}.png"))
...
# Final save
torch.save(encoder.state_dict(), os.path.join(SAVE_PATH, "encoder_final.pth"))
torch.save(decoder.state_dict(), os.path.join(SAVE_PATH, "decoder_final.pth"))
...
print("🎨 Training complete — all models and samples saved to:", SAVE_PATH)
